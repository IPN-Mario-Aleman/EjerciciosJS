...
//console.log(time, courses)
    ...
    for (var i=0; i < courses.length; i++){
        for (var j=i+1; j < courses.length; j++){
            newResult = courses[i] + courses[j]
            //console.log(`${courses[i]} + ${courses[j]} = ${courses[i] + courses[j]}`);      
            //console.log(time)
            //console.log(result)
            //console.log(newResult)
            if (newResult == time){
                return [i, j]
            }else if(newResult < time && newResult > result){
                result = courses[i] + courses[j]
                resultFinal = [i, j]
            }
        }
    }
...
learn(10, [0, 3, 2, 1, 4]) // [0, 2] -> con 10 horas disponibles lo mejor es que completemos los cursos en el índice 0 y 2.
...
learn(8, [8, 1, 1, 2, 5]) // [3, 4] -> usamos el máximo tiempo disponible así que [3, 4] usa 7 horas y el [1, 2] sólo usaría 3 horas.*/

learn(10, [1, 1, 1, 5, 5]) // null -> no nos da tiempo a hacer dos cursos

learn(100, [101, 102, 103, 104, 105, 106]) // null -> no nos da tiempo a hacer dos cursos
