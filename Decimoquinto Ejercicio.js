...
function checkSledJump(heights){
    ...
    for(let i=1;i<heights.length; i++){
      if(heights[i] == heights[i-1]){
        return false // se mantuvo en el aire y no realizo de forma adecuada el salto
      }else if(heights[i] > heights[i-1]){
        sube = sube + 1
        if(heights[i]==heights[heights.length-1]){ 
          return false // si lo ultimo que hizo fue subir y no bajo
        }
      }else if(sube==0){
        return false // si esta bajanado sin siquiera haber subido primero una vez
      }else{
        baja ++
      }
    }
    if(sube > 0 && baja > 0 ){
      return true // solo si subio y baja al menos una vez
    }else{
      return false // si solo subio y no bajo o viceversa una unica vez
    }
  }
