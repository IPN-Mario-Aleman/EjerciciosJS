...
function countPackages(carriers, carrierID){
    //console.log(carriers[1].includes('camila'))
    //console.log(carrierID)
    //console.log(carriers[1])
    const [users,count,workers] = carriers.find( ([cID]) => cID === carrierID )
    console.log([users,count,workers][0])
    return workers.reduce((acc,wID)=>acc+countPackages(carriers,wID),count)
}
